#!/usr/bin/env python3
"""Cross-model attack replay across newer OpenAI models."""
...
def main() -> None:
    ...
    log_path = args.output / "e6_extended_log.jsonl"
    ...
    for model in models:
        ...
        for ticker in TICKERS:
            ...
            try:
                ...
                print(f"  [model err] {model}/{ticker} baseline: {e}", flush=True)
                continue
            bkey = _key(benign)
            print(f"  {ticker} baseline = {benign.action}({benign.quantity})", flush=True)

            log_f.write(json.dumps({
                "model": model, "ticker": ticker, "type": "baseline",
            ...
            for atk in ATTACKS:
                ...
                print(f"    {atk['id']:30}: ASR={asr:.2f} ({n_succ}/{args.repeats})", flush=True)

    log_f.close()

    out = args.output / "e6_extended_results.json"
    ...
    print(f"\n{'='*60}")
    print("Mean ASR by (model, vector):")
    print(f"{'='*60}")
    all_vectors = sorted(set(r["vector"] for r in rows))
    header = f"{'Model':20}" + "".join(f"{v:>8}" for v in all_vectors) + f"{'Mean':>8}"
    print(header)
    print("-" * len(header))
    for model in models:
        vals = []
    ...
    print(f"\nOverall mean ASR by model:")
    for model in models:
        if model in by_m:
            v = by_m[model]
            print(f"  {model:20}: {sum(v)/len(v):.3f} (n={len(v)})")
